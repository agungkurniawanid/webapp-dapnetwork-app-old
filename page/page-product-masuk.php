...
require '../function/tambahproduct.php';
require '../function/hapus-product-masuk.php';

// todo fungsi insert produdct 
if (isset($_POST['submit'])) {
    if (empty($_POST['jumlahmasuk'])) {
        echo "
          <script>
            document.addEventListener('DOMContentLoaded', function () {
              Swal.fire({
                icon: 'info',
                title: 'Oops...',
                text: 'Jumlah produk masuk kosong!',
              ...
              <script>
                document.addEventListener('DOMContentLoaded', function () {
                  Swal.fire({
                    icon: 'success',
                    title: 'Success',
                    text: 'Data product masuk berhasil ditambahkan!',
                    confirmButtonColor: '#3085d6',
                    confirmButtonText: 'OK'
                  }).then((swipe)=>{
                    window.location.href = '../page/page-product-masuk.php';
                  })
                });
              </script>";
        }
...
if (isset($_GET['id_product'])) {
    ...
    if (hapusProductMasuk($id_product) > 0) {
        echo "
          <script>
            document.addEventListener('DOMContentLoaded', function () {
              Swal.fire({
...
// todo fungsi menampilkan all tb_product  tb_product masuk
...
$select_data_hari_ini = "SELECT tb_product_masuk.id_product, 
                                 tb_product.nama_product, 
                                 SUM(tb_product_masuk.jumlah_masuk) as total_jumlah_masuk
                         FROM tb_product_masuk
                         JOIN tb_product ON tb_product_masuk.id_product = tb_product.id_product
                         WHERE DATE(tanggal_masuk) = CURDATE()
                         GROUP BY tb_product_masuk.id_product";

$query_sdhi = mysqli_query($connection_database, $select_data_hari_ini);

// todo fungsi menampilkan data hari kemarin
$select_data_hari_kemarin = "SELECT tb_product_masuk.id_product, 
                                   tb_product.nama_product, 
                                   SUM(tb_product_masuk.jumlah_masuk) as total_jumlah_masuk
                             FROM tb_product_masuk
                             JOIN tb_product ON tb_product_masuk.id_product = tb_product.id_product
                             WHERE DATE(tanggal_masuk) = CURDATE() - 1
                             GROUP BY tb_product_masuk.id_product";
$query_sdhk = mysqli_query($connection_database, $select_data_hari_kemarin);

// todo fungsi menampilkan data minggu ini
$select_data_minggu_ini = "SELECT tb_product_masuk.id_product, 
                                   tb_product.nama_product, 
                                   SUM(tb_product_masuk.jumlah_masuk) as total_jumlah_masuk
                            FROM tb_product_masuk
                            JOIN tb_product ON tb_product_masuk.id_product = tb_product.id_product
                            WHERE WEEK(tanggal_masuk) = WEEK(CURDATE())
                            GROUP BY tb_product_masuk.id_product";
$query_sdmgi = mysqli_query($connection_database, $select_data_minggu_ini);

// todo fungsi menampilkan data bulan ini
$select_data_bulan_ini = "SELECT tb_product_masuk.id_product, 
                                  tb_product.nama_product, 
                                  SUM(tb_product_masuk.jumlah_masuk) as total_jumlah_masuk
                           FROM tb_product_masuk
                           JOIN tb_product ON tb_product_masuk.id_product = tb_product.id_product
...
<body class="w-full h-h-100%">
    <div class="container w-full p-4">
        ...
        <div class="w-full flex items-center justify-between">
            ...
            <!-- untuk judul  -->
            <div>
                <h1 class="text-high font-poppins-600 capitalize">Product Masuk</h1>
            </div>
        </div>
        ...
        <div class="w-full">
            ...
            <div class="w-full mt-4 p-4 shadow-box-shadow-1 rounded-[10px]">
                <h1>Product masuk Hari ini - <strong><?= $hariIndonesia ?></strong></h1>
                <!-- untuk isi data  -->
                        ...
                        <?php while ($data = mysqli_fetch_assoc($query_sdhi)) : ?>
                            <div class="mt-4 p-4 w-full shadow-box-shadow-2 hover:shadow-box-shadow-3 rounded-[10px] transition-all duration-300 ease-out">
                                <form action="" method="post">
                                    <input type="hidden" value="<?= $data['id_product'] ?>" name="idproductmasuk">
                                </form>
                                <div class="header text-high font-poppins-600 capitalize"><?= $data['nama_product'] ?></div>
                                <div class="body text-normales font-poppins-500">Masuk : <?= $data['total_jumlah_masuk'] ?>pcs</div>
                                <div class="flex w-full justify-end gap-gap-10px">
                                    <img onclick="hapusProductMasuk('<?= $data['id_product'] ?>')" type="button" class="w-5 cursor-pointer object-contain" src="../svg/trash.svg" alt="">
                                </div>
                            </div>
                        <?php endwhile; ?>
                    </div>
            ...
            <div class=" mt-4 p-4 shadow-box-shadow-1 rounded-[10px]">
                <h1>Product masuk Hari kemarin - <strong><?= $hariKemarin ?></strong></h1>
                <!-- untuk isi data  -->
                    ...
                    <div class="w-full grid grid-cols-4 gap-gap-10px">
                        ...
                        <?php while ($data = mysqli_fetch_assoc($query_sdhk)) : ?>
                            <div class="mt-4 p-4 w-full shadow-box-shadow-2 hover:shadow-box-shadow-3 rounded-[10px] transition-all duration-300 ease-out">
                                ...
                                <div class="body text-normales font-poppins-500">Masuk : <?php echo $data['total_jumlah_masuk'] ?>pcs</div>
                                <div class="flex w-full justify-end gap-gap-10px">
                                    <img onclick="hapusProductMasuk('<?= $data['id_product'] ?>')" type="button" class="w-5 cursor-pointer object-contain" src="../svg/trash.svg" alt="">
                                </div>
                            </div>
            ...
            <!-- tanggal minggu ini  -->
            <div class=" mt-4 p-4 shadow-box-shadow-1 rounded-[10px]">
                <h1>Product masuk minggu ini</h1>
                <!-- untuk isi data  -->
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="font-poppins-600 text-high capitalize" id="exampleModalLabel">Tambah Product Masuk</h1>
                    ...
                    <div class="modal-body">
                        <form action="" method="post">
                            ...
                            <div class="mb-3">
                                <label for="message-text" class="col-form-label">Jumlah Masuk</label>
                                <input type="number" class="form-control" id="message-text" name="jumlahmasuk">
                            </div>
        ...
        function hapusProductMasuk(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = 'page-product-masuk.php?id_product=' + id;
                }
            })
        };
    </script>
